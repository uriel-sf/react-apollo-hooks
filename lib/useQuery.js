...
function useQuery(query, _temp) {
  ...
  var currentResult = (0, _react.useMemo)(function () {
    ...
    var networkStatus = result.networkStatus === _apolloClient.NetworkStatus.error && !result.error && !result.errors ? _apolloClient.NetworkStatus.ready : result.networkStatus;
    return _extends({}, helpers, {
      data: data,
  ...
  (0, _react.useEffect)(function () {
    ...
    function invalidateErrorResult() {
      unsubscribe();
      var lastError = observableQuery.getLastError();
    ...
    function subscribe() {
      subscription = observableQuery.subscribe(invalidateCurrentResult, invalidateErrorResult);
    }
